...
export async function executeArb(connection: Connection, opp: Opportunity, payer: { publicKey: PublicKey; sign: (tx: VersionedTransaction)=>Promise<void>; }) {
  ...
  const buildIx = async (which: "orca"|"raydium", aToB: boolean) =>
    which === "orca"
      ? buildSwapIxOrca(opp.pair, opp.sizeUsd, aToB)
      : buildSwapIxRaydium(opp.pair, opp.sizeUsd, aToB);

  const ixBuy  = await buildIx(opp.dexBuy,  true);
  const ixSell = await buildIx(opp.dexSell, false);

  const flArgs: FlashLoanArgs = {
    amount: BigInt(Math.floor(opp.sizeUsd)),  // NOTE: unità/mint corrette in produzione
    instructions: [ixBuy, ixSell]
  };
  ...
  const res = await sendBundle([tx], chooseTipLamports());
  return res;
}
